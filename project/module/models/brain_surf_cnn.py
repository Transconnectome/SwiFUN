...
class Up3D(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = ResBlock3D(in_ch, out_ch)
...
class BrainSurfCNN3D(nn.Module):
    def __init__(self, in_ch, out_ch, fdim=64):
        super().__init__()
        self.in_conv = nn.Sequential(
            nn.Conv3d(in_ch, fdim, kernel_size=3, padding=1),
            nn.ReLU(inplace=True)
        )
        self.down1 = Down3D(fdim, fdim*2)
        self.down2 = Down3D(fdim*2, fdim*4)
        self.down3 = Down3D(fdim*4, fdim*8)
        self.down4 = Down3D(fdim*8, fdim*16)

        self.up1 = Up3D(fdim*16, fdim*8)  # in_ch = fdim*16 because of concatenation
        self.up2 = Up3D(fdim*8, fdim*4)   # in_ch = fdim*8 because of concatenation
        self.up3 = Up3D(fdim*4, fdim*2)     # in_ch = fdim*4 because of concatenation
        self.up4 = Up3D(fdim*2, fdim)     # in_ch = fdim*2 because of concatenation

        self.out_conv = nn.Conv3d(fdim, out_ch, kernel_size=1)
